...
On a heavily modded **BG2EE** install (`en_US`, Placeholders configured) scanned after
companion banter / `pdialog` / side-chain coverage landed, Attribution counted roughly:

| Bucket | Count | Share of this scan’s spoken lines |
|--------|------:|----------------------------------:|
| Ready to generate / pack (Ready + already exported) | ~33,300 | ~75% |
| Already voiced (official VO) | ~10,200 | ~23% |
| Shared different-voice (deferred) | ~900 | ~2% |
| **Spoken lines found by this scan** | **~44,400** | |

(~250 additional **Non-spoken** skipped lines sit outside that table.) Within *those*
spoken lines, pack-eligible Ready lines plus official VO are about **~98%**; the remaining
~2% are shared-strref deferrals the tool deliberately does not auto-patch.

That is **not** a claim that every NPC line in Baldur’s Gate II is covered. The
percentages only describe how the current Attribution scan classified the dialogue it
found (CRE-owned DLGs + companion tables above). Orphan DLGs, player replies, journals,
dream scripts, barks, and anything the scan does not attach to a speaker stay out of
scope — and a quieter install (fewer mods / different locale) will show smaller totals.
Always treat Attribution counts as **install- and scan-specific**, and note the
fingerprint/mod setup when sharing a pack.
